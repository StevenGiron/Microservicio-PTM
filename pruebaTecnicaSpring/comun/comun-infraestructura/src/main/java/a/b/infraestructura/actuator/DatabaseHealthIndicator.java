...
package a.b.infraestructura.actuator;
...
public class DatabaseHealthIndicator extends AbstractReactiveHealthIndicator {
     ...
     *                          validation
     */
    public DatabaseHealthIndicator(ConnectionFactory connectionFactory, String validationQuery) {
    ...
    private Mono<Health> validateWithConnectionValidation(Health.Builder builder) {
        ...
        return connectionValidation.map(valid -> builder.status(Boolean.TRUE.equals(valid) ? Status.UP : Status.DOWN).build());
    }
