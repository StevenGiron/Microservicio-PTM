package a.b.infraestructura.r2dbc;

import a.b.infraestructura.excepcion.ExcepcionTecnica;
...
public class CustomRRDBCTemplate {
    ...
    public Mono<Long> create(Object object, String sql) {
        DatabaseClient.GenericExecuteSpec query = databaseClient.sql(sql)
                .filter((statement, executeFunction) -> statement.returnGeneratedValues("id").execute());
        query = buildSql(object, query);
        return query.fetch().first().map(r -> (Long) r.get("id"));
    }

    public Mono<Long> update(Object object, String sql) {
        DatabaseClient.GenericExecuteSpec query = databaseClient.sql(sql);
        query = buildSql(object, query);
    ...
    public <R> Mono<R> query(Map<String, Object> parameters, Function<Row, R> mapper, String sql) {
        DatabaseClient.GenericExecuteSpec query = databaseClient.sql(sql);
        query = insertParameters(parameters, query);
        return query.map(t -> mapper.apply((Row) t)).one();
    }
